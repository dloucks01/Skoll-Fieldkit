"""Remediation knowledge base for gen_report.py — keyed by vector_type.

Each finding in the findings file names a vector_type; the report auto-fills severity, CWE,
a standard description, and (the part the customer needs) the REMEDIATION, from here. The
operator supplies the target-specific bits (host, evidence, and the exact steps/commands run).
